<script lang="ts">
	...
	import type { ICreateDpa } from '$lib/interface/create/ICreateDpa';
	...
	import { Create } from '$lib/api/pembangunan/dpa/informasi';

	let myBreadItems = [
		{ label: 'Home', href: '/dashboard' },
		{ label: 'DPA', href: '/pembangunan/dpa' },
		{ label: 'Informasi DPA', href: '/pembangunan/dpa/add' }
	];

	let stepperData: IStepper[] = [
		{
			label: 'Informasi DPA',
			href: '/pembangunan/dpa/add'
		},
		{
			label: 'Rincian DPA',
			href: ''
	...
	onMount(async () => {
		...
		const tahunActive = await GetTahunActive();
		if (typeof tahunActive != 'undefined') {
			DataTahunSelected = {
				label: tahunActive.tahun,
				value: tahunActive.id
			};
			CreateDpa.tahunId = tahunActive.id;
		}
	});

	async function addDpa(e: any) {
		isLoadingAdd = true;
		if (tenant.status == '1') {
			if (typeof tenant.dinas != 'undefined') {
				CreateDpa.dinasId = tenant.dinas;
			}
		} else {
			if (CreateDpa.dinasId == '') {
				errorMessage['dinasId'] = 'Dinas harus diisi';
			}
		}
		const responses: any = await Create(CreateDpa);
		if (responses.status >= 200 && responses.status <= 300) {
			...
			isLoadingAdd = false;
			goto('/pembangunan/dpa/add/' + responses.data.id + '/rincian');
		} else {
			//notif
	...
	function handleInputJumlahAlokasi(event: any) {
		CreateDpa.jumlahAlokasi = parseFloat(event.target.value.replace(/\D/g, '')) || 0;
		formatJumlahAlokasi = formatCurrency(CreateDpa.jumlahAlokasi);
	}

	const handleDinasSelect = async (event: any) => {
		CreateDpa.dinasId = event.detail.value.value.toString();
	};
</script>
...
<div class="">
	...
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		...
		<form on:submit|preventDefault={addDpa} class="flex flex-col gap-3">
