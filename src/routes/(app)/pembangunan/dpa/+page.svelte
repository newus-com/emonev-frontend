<script lang="ts">
	...
	import { List } from '$lib/api/pembangunan/dpa/informasi';
	...
	export let dataTable = {
		search: {
			column: ['dpa_pembangunan.noDpa', 'urusan.nomenklatur'],
			value: ''
		},
		order: [
			{
				column: 'dpa_pembangunan.id',
				dir: 'desc'
			}
		],
		length: 10,
		...
		<div class="">
				...
				<div class="w-full">
					<div class="grid grid-cols-2">
						...
						<div class="flex justify-end gap-2">
							...
							{#if checkPermission('C_DPA_PEMBANGUNAN', permission)}
								{#if !isLoadingAdd}
									<Button
										size="sm"
										on:click={buttonAdd}
			...
			<div class="rounded-md overflow-hidden border">
				<Table class="rounded-md overflow-hidden">
					...
					<TableBody>
						{#if checkPermission('R_DPA_PEMBANGUNAN', permission)}
							{#if isLoading == true}
							...
							{:else}
								{#each Dpa as item (item.id)}
									<TableBodyRow class="text-center">
										...
										<TableBodyCell tdClass="p-0 m-0">
											<Button
												href="/pembangunan/dpa/add/{item.id}"
