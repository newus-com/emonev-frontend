<script lang="ts">
	...
	import { Update } from '$lib/api/dpa/rincian';
	...
	async function addRincian(e: any) {		
		...
		isLoadingAdd = true;
		const responses: any = await Update(dataRincian);
		if (responses.status >= 200 && responses.status <= 300) {
			...
			isLoadingAdd = false;
			goto('/anggaran/dpa/add/' + responses.data.id + '/rincian/subKegiatan');
		} else {
			//notif
