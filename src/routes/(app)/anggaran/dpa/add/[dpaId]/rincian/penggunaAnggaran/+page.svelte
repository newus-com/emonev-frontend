<script lang="ts">
	...
	import { List, Update } from '$lib/api/dpa/penggunaAnggaran';
	...
	interface typePenggunaAnggaran {
		id: any;
		nama: string;
		nip: string;
		jabatan: string;
	...
	let penggunaAnggaran: typePenggunaAnggaran[] = [];

	async function getPenggunaAnggaran() {
		if (tenant.status == '1') {
			if (typeof tenant.dinas != 'undefined') {
		...
		const dataPenggunaAnggaran = await List(dataTable);
		if (dataPenggunaAnggaran.status == 200) {
			penggunaAnggaran = dataPenggunaAnggaran.data.penggunaAnggaran;
		} else {
			//notif
			if (typeof dataPenggunaAnggaran.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataPenggunaAnggaran.message,
						timeout: 3000
					}
				]);
			}
	...
	async function onEdit() {
		...
		data['penggunaAnggaran'] = penggunaAnggaran;

		console.log(data['penggunaAnggaran']);
	...
	const buttonDelete = () => {
		errorMessage = {};
		penggunaAnggaran = penggunaAnggaran.filter((item) => item.id !== deleteValue.id);
	};
			...
			<div class="">
				...
				<div class="rounded-md overflow-hidden border">
					<Table class="rounded-md overflow-hidden">
						...
						<TableBody>
							{#each penggunaAnggaran as item (item.id)}
