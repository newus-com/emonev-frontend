<script lang="ts">
	...
	let DataBulanFilter: ISelect[] | boolean;
	let BulanSelectedFilter: ISelect;
	...
	const handleTahunSelectFilter = async (event: any) => {
		...
		TahunSelectedFilter = {
			label: event.detail.value.label,
	...
	const handleBulanSelectFilter = async (event: any) => {
		dataTableSubDpa.bulan = event.detail.value.value;
		BulanSelectedFilter = {
			label: event.detail.value.label,
			value: event.detail.value.value
		};
	...
	onMount(async () => {
		...
		DataBulanFilter.unshift(BulanSelectedFilter);
	});
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Laporan</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		<div class="">
						...
						<div class="grid grid-cols-2 gap-2">
							{#if tenant.status == '0'}
								<Label class="space-y-2 mb-2">
									<span class="flex gap-2">
										Perangkat Daerah
										{#if typeof DinasSelectedFilter != 'undefined'}
											{#if DinasSelectedFilter.value == '0'}
												<Toggle size="small" color="blue">Detail</Toggle>
											{/if}
										{/if}
									</span>
									<Select2
							...
							<div class="grid grid-cols-2 gap-2">
								<Label class="space-y-2 mb-2">
									<span class="flex gap-2">
										Bulan
										<Toggle size="small" color="blue">Sampai Dengan</Toggle>
									</span>
									<Select2
										placeholder="Pilih Bulan"
										name="bulanFilter"
										data={DataBulanFilter}
										selected={BulanSelectedFilter}
										on:Select2Change={handleBulanSelectFilter}
									/>
								</Label>
								<Label class="space-y-2 mb-2">
									<span>Tahun</span>
			...
			<p class="font-bold text-md text-black text-center">
				REKAPITULASI LAPORAN KEUANGAN DAN PENCAPAIAN KINERJA SEKRETARIAT DAERAH
			</p>
			<p class="font-bold text-md text-black text-center">
				APBD KABUPATEN PESISIR BARAT TAHUN ANGGARAN {typeof TahunSelectedFilter != 'undefined'
					? TahunSelectedFilter.label
					: '-'}
			</p>
			<p class="font-bold text-md text-black text-center">
				Bulan {typeof BulanSelectedFilter != 'undefined' ? BulanSelectedFilter.label : '-'}
			</p>
			<p class="font-bold text-md text-black text-center" />
			<small
				>Perangkat Daerah: {typeof DinasSelectedFilter != 'undefined'
					? DinasSelectedFilter.label
					: '-'}</small
			>
				...
				<Table class="rounded-md relative overflow-x-auto">
					<tr class="text-center bg-[#1E40AF] text-white p-0">
						<TableHeadCell padding="p-1" class="text-xs border border-slate-400 " rowspan={3}
							>Program / Kegiatan / Sub Kegiatan</TableHeadCell
						>
					...
					<TableBody>
						...
						{:else if isLoading == true}
							<STable totalColumn={19} totalRow={10} />
						{:else}
