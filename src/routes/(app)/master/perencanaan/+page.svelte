<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/perencanaan/urusan';
	import {
		Button,
		Modal,
		Label,
	...
	let dataTable = {
		search: {
			column: ['urusan.kode', 'urusan.nomenklatur'],
			value: ''
		},
		order: [
			{
	...
	async function getUrusan() {
		const dataUrusan = await List(dataTable);
		if (dataUrusan.status == 200) {
			Urusan = dataUrusan.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
			...
			if (Urusan.length != 0) {
				totalPages = dataUrusan.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Urusan</div>
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				...
				<P size="xs" height="relaxed">
					<small class="text-[#FF3636] pt-0 mt-0">**</small> Klik Kode atau Nomenklatur jika ingin ke data bidang.
				</P>
			</Blockquote>
						...
						{#each Urusan as item}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/perencanaan/bidang/{item.id}">{item.kode}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/perencanaan/bidang/{item.id}">{item.nomenklatur}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0">
									<Button
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Urusan</h3>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Urusan</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
