<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/perencanaan/subKegiatan';
	import {
		Button,
		Modal,
		Label,
	...
	import { One } from '$lib/api/perencanaan/kegiatan';
	...
	let myBreadItems = [
		...
		{ label: 'Bidang', href: '/master/perencanaan/bidang/' + $page.params.urusanId },
		{
			label: 'Program',
			href:
				'/master/perencanaan/bidang/' + $page.params.urusanId + '/program/' + $page.params.bidangId
		},
		{
			label: 'Kegiatan',
			href:
				'/master/perencanaan/bidang/' +
				$page.params.urusanId +
				'/program/' +
				$page.params.bidangId +
				'/kegiatan/' +
	...
	let dataTable = {
		...
		length: 10,
		page: 0,
		kegiatanId: $page.params.kegiatanId
	};
	...
	let SatuanSelected: ISelect;

	async function getSubKegiatan() {
		const dataSubKegiatan = await List(dataTable);
	...
	async function getOneKegiatan(id: string) {
		const dataKegiatan = await One(id);
		if (dataKegiatan.status == 200) {
			Kegiatan = {
				id: dataKegiatan.data.id,
				kode: dataKegiatan.data.kode,
				nomenklatur: dataKegiatan.data.nomenklatur,
				programId: dataKegiatan.data.programId
			};
		} else {
			//notif
			if (typeof dataKegiatan.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataKegiatan.message,
						timeout: 3000
					}
				]);
			}
	...
	onMount(async () => {
		...
		await getOneKegiatan($page.params.kegiatanId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['kegiatanId'] = $page.params.kegiatanId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['kegiatanId'] = editValue.kegiatanId;
	...
	const buttonEdit = () => {
		...
		GetListSatuan('')
			.then((result) => {
				DataSatuan = result;
				for (const item of DataSatuan) {
					if (editValue.satuanId === item.value) {
						SatuanSelected = item;
						break;
					}
				}
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Sub Kegiatan</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed"><b>Data sub kegiatan dari kegiatan:</b></P>

				<P size="xs" height="relaxed">
					"{Kegiatan.kode} | {Kegiatan.nomenklatur}"
				</P>
			</Blockquote>
			...
			<Table class="rounded-md overflow-hidden">
				<TableHead class="text-center bg-[#1E40AF] text-white">
					...
					<TableHeadCell>Kinerja</TableHeadCell>
				...
				<TableBody>
					...
					{:else if SubKegiatan.length === 0}
						<TableBodyRow class="text-center">
							<TableBodyCell colspan="6">No Data ...</TableBodyCell>
						</TableBodyRow>
					{:else}
						{#each SubKegiatan as item}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0">{item.kode}</TableBodyCell>
								<TableBodyCell tdClass="p-0 m-0">{item.nomenklatur}</TableBodyCell>
								<TableBodyCell tdClass="p-0 m-0">{item.satuan}</TableBodyCell>
								<TableBodyCell tdClass="p-0 m-0">{item.kinerja}</TableBodyCell>
								<TableBodyCell tdClass="p-0 m-0">{item.indikator}</TableBodyCell>
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		...
		<Label class="space-y-2">
			...
			/>
			{#if typeof errorMessage.kinerja !== 'undefined'}
				<Error messageError={errorMessage.kinerja} typeError="error" />
			{/if}
		</Label>
		<Label class="space-y-2">
			<span>Indikator</span>
			...
			/>
			{#if typeof errorMessage.indikator !== 'undefined'}
				<Error messageError={errorMessage.indikator} typeError="error" />
			{/if}
		</Label>
		{#if !isLoadingAdd}
	...
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		...
		<Label class="space-y-2">
			...
			<Input
				...
				name="indikator"
				placeholder="Masukkan Indikator"
				bind:value={editValue.indikator}
			/>
			{#if typeof errorMessage.indikator !== 'undefined'}
				<Error messageError={errorMessage.indikator} typeError="error" />
			{/if}
