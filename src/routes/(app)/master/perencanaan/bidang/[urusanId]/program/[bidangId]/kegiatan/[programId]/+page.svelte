<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/perencanaan/kegiatan';
	import {
		Button,
		Modal,
		Label,
	...
	import { One } from '$lib/api/perencanaan/program';
	...
	let myBreadItems = [
		...
		{ label: 'Kegiatan', href: '/master/perencanaan/bidang/' + $page.params.urusanId + '/program/' + $page.params.bidangId + '/kegiatan/' + $page.params.programId},
	];
	...
	let stepperData: IStepper[] = [
		...
		{
			label: 'Kegiatan',
			href: '/master/perencanaan/bidang/' + $page.params.urusanId + '/program/' + $page.params.bidangId + '/kegiatan/' + $page.params.programId
		},
		{
			label: 'Sub Kegiatan',
			href: ''
	...
	let dataTable = {
		search: {
			column: ['kegiatan.kode', 'kegiatan.nomenklatur'],
			value: ''
		},
		order: [
			{
	...
	async function getKegiatan() {
		const dataKegiatan = await List(dataTable);
		if (dataKegiatan.status == 200) {
			Kegiatan = dataKegiatan.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
			...
			if (Kegiatan.length != 0) {
				totalPages = dataKegiatan.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
	...
	async function getOneProgram(id: string) {
		const dataProgram = await One(id);
		if (dataProgram.status == 200) {
			Program = {
				id: dataProgram.data.id,
				kode: dataProgram.data.kode,
				nomenklatur: dataProgram.data.nomenklatur,
				bidangId: dataProgram.data.bidangId,
			};
		} else {
			//notif
	...
	onMount(async () => {
		isLoading = true;
		await getKegiatan();
		await getOneProgram($page.params.programId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['programId'] = $page.params.programId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['programId'] = editValue.programId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Kegiatan</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed"><b>Data kegiatan dari program:</b></P>

				<P size="xs" height="relaxed">
					"{Program.kode} | {Program.nomenklatur}"
				</P>
				<P size="xs" class="text-[#FF3636]" height="relaxed">
					Perhatian :
				</P>
				<P size="xs" height="relaxed">
					<small class="text-[#FF3636] pt-0 mt-0">**</small> Klik Kode atau Nomenklatur jika ingin ke data sub kegiatan.
				</P>
			</Blockquote>
			...
			<Table class="rounded-md overflow-hidden">
				...
				<TableBody>
					...
					{:else}
						{#each Kegiatan as item}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/perencanaan/bidang/{$page.params.urusanId}/program/{$page.params.bidangId}/kegiatan/{$page.params.programId}/subKegiatan/{item.id}"
										>{item.kode}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Kegiatan</h3>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Kegiatan</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
