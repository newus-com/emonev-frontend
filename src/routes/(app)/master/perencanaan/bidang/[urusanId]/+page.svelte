<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/perencanaan/bidang';
	import {
		Button,
		Modal,
		Label,
	...
	import { One } from '$lib/api/perencanaan/urusan';
	...
	let dataTable = {
		search: {
			column: ['bidang.kode', 'bidang.nomenklatur'],
			value: ''
		},
		order: [
			{
	...
	async function getBidang() {
		const dataBidang = await List(dataTable);
		if (dataBidang.status == 200) {
			Bidang = dataBidang.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
				nomenklatur: item.nomenklatur,
				urusanId: item.urusanId
			}));

			if (Bidang.length != 0) {
				totalPages = dataBidang.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
	...
	async function getOneUrusan(id: string) {
		const dataUrusan = await One(id);
		if (dataUrusan.status == 200) {
			Urusan = {
				id: dataUrusan.data.id,
				kode: dataUrusan.data.kode,
				nomenklatur: dataUrusan.data.nomenklatur
			};
		} else {
			//notif
			if (typeof dataUrusan.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataUrusan.message,
						timeout: 3000
					}
				]);
			}
	...
	onMount(async () => {
		isLoading = true;
		await getBidang();
		await getOneUrusan($page.params.urusanId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['urusanId'] = $page.params.urusanId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['urusanId'] = editValue.urusanId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Bidang</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed"><b>Data bidang dari urusan:</b></P>

				<P size="xs" height="relaxed">
					"{Urusan.kode} | {Urusan.nomenklatur}"
				</P>
				<P size="xs" class="text-[#FF3636]" height="relaxed">
					Perhatian :
				</P>
				<P size="xs" height="relaxed">
					<small class="text-[#FF3636] pt-0 mt-0">**</small> Klik Kode atau Nomenklatur jika ingin ke data program.
				</P>
			</Blockquote>
						...
						{#each Bidang as item}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/perencanaan/bidang/{$page.params.urusanId}/program/{item.id}"
										>{item.kode}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Bidang</h3>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Bidang</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
