<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/rekening/subRincianObjekRekening';
	import { One } from '$lib/api/rekening/rincianObjekRekening';
	import {
		Button,
		Modal,
		Label,
	...
	import type { ISubRincianObjekRekening } from '$lib/interface/ISubRincianObjekRekening.js';
	import type { IRincianObjekRekening } from '$lib/interface/IRincianObjekRekening.js';
	...
	let dataTable = {
		search: {
			column: [
				...
				'sub_rincian_objek_rekening.deskripsiAkun'
			],
			value: ''
		},
		order: [
			{
				column: 'sub_rincian_objek_rekening.id',
				dir: 'desc'
			}
		],
		length: 10,
		page: 0,
		rincianObjekRekeningId: $page.params.rincianObjekRekeningId
	};
	...
	let myBreadItems = [
		...
		},
		{
			...
			href:
				...
				$page.params.jenisRekeningId +
				'/rincianObjek/' +
				$page.params.objekRekeningId
		},
		{
			label: 'Sub Rician Objek',
			href:
				'/master/rekening/kelompok/' +
				$page.params.akunRekeningId +
				'/jenis/' +
				...
				$page.params.objekRekeningId +
				'/subRincianObjek/' +
				$page.params.rincianObjekRekeningId
		}
	];

	let SubRincianObjekRekening: ISubRincianObjekRekening[] = [];

	let RincianObjekRekening: IRincianObjekRekening = {
		id: '',
		kode: '',
		uraianAkun: '',
		deskripsiAkun: '',
	...
	async function getSubRincianObjekRekening() {
		const dataSubRincianObjekRekening = await List(dataTable);
		if (dataSubRincianObjekRekening.status == 200) {
			SubRincianObjekRekening = dataSubRincianObjekRekening.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
				uraianAkun: item.uraianAkun,
				deskripsiAkun: item.deskripsiAkun,
				rincianObjekRekeningId: item.rincianObjekRekeningId
			}));

			if (SubRincianObjekRekening.length != 0) {
				totalPages = dataSubRincianObjekRekening.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
			...
			//notif
			if (typeof dataSubRincianObjekRekening.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataSubRincianObjekRekening.message,
						timeout: 3000
					}
				]);
			}
	...
	async function getOneRincianObjekRekening(id: string) {
		const dataRincianObjekRekening = await One(id);
		if (dataRincianObjekRekening.status == 200) {
			RincianObjekRekening = {
				id: dataRincianObjekRekening.data.id,
				kode: dataRincianObjekRekening.data.kode,
				uraianAkun: dataRincianObjekRekening.data.uraianAkun,
				deskripsiAkun: dataRincianObjekRekening.data.deskripsiAkun,
				objekRekeningId: dataRincianObjekRekening.data.objekRekeningId
			};
		} else {
			//notif
			if (typeof dataRincianObjekRekening.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataRincianObjekRekening.message,
						timeout: 3000
					}
				]);
			}
	...
	onMount(async () => {
		isLoading = true;
		await getSubRincianObjekRekening();
		await getOneRincianObjekRekening($page.params.rincianObjekRekeningId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['rincianObjekRekeningId'] = $page.params.rincianObjekRekeningId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['rincianObjekRekeningId'] = editValue.rincianObjekRekeningId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Sub Rincian Objek Rekening</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		<div class="mb-3">
			<Stepper stepperItems={stepperData} active={5} />
		</div>
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed">
					<b>Data sub rincian objek rekening dari rincian objek rekening: </b>
				</P>
				<P size="xs" height="relaxed">
					"{RincianObjekRekening.kode} | {RincianObjekRekening.uraianAkun} | {RincianObjekRekening.deskripsiAkun}"
				</P>
			</Blockquote>
			...
			<Table class="rounded-md overflow-hidden">
				...
				<TableBody>
					...
					{:else if SubRincianObjekRekening.length === 0}
						<TableBodyRow class="text-center">
							<TableBodyCell colspan="4">No Data ...</TableBodyCell>
						</TableBodyRow>
					{:else}
						{#each SubRincianObjekRekening as item (item.id)}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									>{item.kode}</TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									>{item.uraianAkun}</TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									>{item.deskripsiAkun}</TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0">
									<Button
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">
			Tambah Sub Rincian Objek Rekening
		</h3>
		...
		<Label class="space-y-2">
			<span>Uraian Sub Rincian Objek Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="uraianAkun"
				placeholder="Masukkan Uraian Sub Rincian Objek Rekening"
			/>
		...
		<Label class="space-y-2">
			<span>Deskripsi Sub Rincian Objek Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="deskripsiAkun"
				placeholder="Masukkan Deskripsi Sub Rincian Objek Rekening"
			/>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">
			Ubah Sub Rincian Objek Rekening
		</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
