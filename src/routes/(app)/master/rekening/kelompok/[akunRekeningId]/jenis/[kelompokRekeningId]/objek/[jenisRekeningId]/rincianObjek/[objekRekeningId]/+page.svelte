<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/rekening/rincianObjekRekening';
	import { One } from '$lib/api/rekening/objekRekening';
	import {
		Button,
		Modal,
		Label,
	...
	let dataTable = {
		search: {
			column: [
				...
				'rincian_objek_rekening.deskripsiAkun'
			],
			value: ''
		},
		order: [
			{
				column: 'rincian_objek_rekening.id',
				dir: 'desc'
			}
		],
		length: 10,
		page: 0,
		objekRekeningId: $page.params.objekRekeningId
	};
	...
	async function getRincianObjekRekening() {
		const dataRincianObjekRekening = await List(dataTable);
		if (dataRincianObjekRekening.status == 200) {
			RincianObjekRekening = dataRincianObjekRekening.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
				uraianAkun: item.uraianAkun,
				deskripsiAkun: item.deskripsiAkun,
				objekRekeningId: item.objekRekeningId
			}));

			if (RincianObjekRekening.length != 0) {
				totalPages = dataRincianObjekRekening.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
	...
	async function getOneObjekRekening(id: string) {
		const dataObjekRekening = await One(id);
		if (dataObjekRekening.status == 200) {
			ObjekRekening = {
				id: dataObjekRekening.data.id,
				kode: dataObjekRekening.data.kode,
				uraianAkun: dataObjekRekening.data.uraianAkun,
				deskripsiAkun: dataObjekRekening.data.deskripsiAkun,
				jenisRekeningId: dataObjekRekening.data.jenisRekeningId
			};
		} else {
			//notif
	...
	onMount(async () => {
        console.log($page.params.objekRekeningId);
        
		isLoading = true;
		await getRincianObjekRekening();
		await getOneObjekRekening($page.params.objekRekeningId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['objekRekeningId'] = $page.params.objekRekeningId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['objekRekeningId'] = editValue.objekRekeningId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Rincian Objek Rekening</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		<div class="mb-3">
			<Stepper stepperItems={stepperData} active={4} />
		</div>
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed">
					<b>Data rincian objek rekening dari objek rekening: </b>
				</P>
				<P size="xs" height="relaxed">
					"{ObjekRekening.kode} | {ObjekRekening.uraianAkun} | {ObjekRekening.deskripsiAkun}"
				</P>
				...
				<P size="xs" height="relaxed">
					...
					ingin ke data sub rincian objek rekening.
				</P>
			</Blockquote>
			...
			<Table class="rounded-md overflow-hidden">
				...
				<TableBody>
					...
					{:else if RincianObjekRekening.length === 0}
						<TableBodyRow class="text-center">
							<TableBodyCell colspan="4">No Data ...</TableBodyCell>
						</TableBodyRow>
					{:else}
						{#each RincianObjekRekening as item (item.id)}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a
										href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page
											...
											.jenisRekeningId}/rincianObjek/{$page.params.objekRekeningId}/subRincianObjek/{item.id}">{item.kode}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a
										href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page
											.params.kelompokRekeningId}/objek/{$page.params
											.jenisRekeningId}/rincianObjek/{$page.params.objekRekeningId}/subRincianObjek/{item.id}">{item.uraianAkun}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a
										href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page
											.params.kelompokRekeningId}/objek/{$page.params
											.jenisRekeningId}/rincianObjek/{$page.params.objekRekeningId}/subRincianObjek/{item.id}">{item.deskripsiAkun}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0">
									<Button
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Rincian Objek Rekening</h3>
		...
		<Label class="space-y-2">
			<span>Uraian Rincian Objek Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="uraianAkun"
				placeholder="Masukkan Uraian Rincian Objek Rekening"
			/>
		...
		<Label class="space-y-2">
			<span>Deskripsi Rincian Objek Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="deskripsiAkun"
				placeholder="Masukkan Deskripsi Rincian Objek Rekening"
			/>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Rincian Objek Rekening</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
