<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/rekening/jenisRekening';
	import { One } from '$lib/api/rekening/kelompokRekening';
	import {
		Button,
		Modal,
		Label,
	...
	let dataTable = {
		search: {
			column: [
				...
				'jenis_rekening.deskripsiAkun'
			],
			value: ''
		},
		order: [
			{
				column: 'jenis_rekening.id',
				dir: 'desc'
			}
		],
		length: 10,
		page: 0,
		kelompokRekeningId: $page.params.kelompokRekeningId
	};
	...
	let myBreadItems = [
		...
		{ label: 'Jenis', href: '/master/rekening/kelompok/' + $page.params.akunRekeningId + '/jenis/' + $page.params.kelompokRekeningId}
	];
	...
	let stepperData: IStepper[] = [
		...
		},
		{
			...
			href: '/master/rekening/kelompok/' + $page.params.akunRekeningId + '/jenis/' + $page.params.kelompokRekeningId
		},
		{
	...
	async function getJenisRekening() {
		const dataJenisRekening = await List(dataTable);
		if (dataJenisRekening.status == 200) {
			JenisRekening = dataJenisRekening.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
				uraianAkun: item.uraianAkun,
				deskripsiAkun: item.deskripsiAkun,
				kelompokRekeningId: item.kelompokRekeningId
			}));

			if (JenisRekening.length != 0) {
				totalPages = dataJenisRekening.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
	...
	async function getOneKelompokRekening(id: string) {
		const dataKelompokRekening = await One(id);
		if (dataKelompokRekening.status == 200) {
			KelompokRekening = {
				id: dataKelompokRekening.data.id,
				kode: dataKelompokRekening.data.kode,
				uraianAkun: dataKelompokRekening.data.uraianAkun,
				deskripsiAkun: dataKelompokRekening.data.deskripsiAkun,
				akunRekeningId: dataKelompokRekening.data.akunRekeningId,
			};
		} else {
			//notif
	...
	onMount(async () => {
		isLoading = true;
		await getJenisRekening();
		await getOneKelompokRekening($page.params.kelompokRekeningId);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['kelompokRekeningId'] = $page.params.kelompokRekeningId;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['kelompokRekeningId'] = editValue.kelompokRekeningId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Jenis Rekening</div>
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed">
					<b>Data jenis rekening dari kelompok rekening: </b>
				</P>
				<P size="xs" height="relaxed">
					"{KelompokRekening.kode} | {KelompokRekening.uraianAkun} | {KelompokRekening.deskripsiAkun}"
				</P>
			...
			<Table class="rounded-md overflow-hidden">
				...
				<TableBody>
					...
					{:else if JenisRekening.length === 0}
						<TableBodyRow class="text-center">
							<TableBodyCell colspan="4">No Data ...</TableBodyCell>
						</TableBodyRow>
					{:else}
						{#each JenisRekening as item (item.id)}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page.params.kelompokRekeningId}/objek/{item.id}">{item.kode}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page.params.kelompokRekeningId}/objek/{item.id}">{item.uraianAkun}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a href="/master/rekening/kelompok/{$page.params.akunRekeningId}/jenis/{$page.params.kelompokRekeningId}/objek/{item.id}">{item.deskripsiAkun}</a
									></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0">
									<Button
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Jenis Rekening</h3>
		...
		<Label class="space-y-2">
			<span>Uraian Jenis Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="uraianAkun"
				placeholder="Masukkan Uraian Jenis Rekening"
			/>
		...
		<Label class="space-y-2">
			<span>Deskripsi Jenis Rekekning</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="deskripsiAkun"
				placeholder="Masukkan Deskripsi Jenis Rekekning"
			/>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Jenis Rekening</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
