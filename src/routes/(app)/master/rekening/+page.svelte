<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/rekening/akunRekening';
	import {
		Button,
		Modal,
		Label,
	...
	import type { IAkunRekening } from '$lib/interface/IAkunRekening';
	...
	let dataTable = {
		search: {
			column: ['akun_rekening.kode', 'akun_rekening.uraianAkun', 'akun_rekening.deskripsiAkun'],
			value: ''
		},
		order: [
			{
				column: 'akun_rekening.id',
				dir: 'desc'
			}
		],
		length: 10,
	...
	async function getAkunRekening() {
		const dataAkunRekening = await List(dataTable);
		if (dataAkunRekening.status == 200) {
			AkunRekening = dataAkunRekening.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
			...
			if (AkunRekening.length != 0) {
				totalPages = dataAkunRekening.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Akun Rekening</div>
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				...
				<P size="xs" height="relaxed">
					<small class="text-[#FF3636] pt-0 mt-0">**</small> Klik Kode, Uraian Akun, Deskripsi Akun jika ingin ke data kelompok rekening.
				</P>
			</Blockquote>
			...
			<Table class="rounded-md overflow-hidden">
				...
				<TableBody>
					...
					{:else if AkunRekening.length === 0}
						<TableBodyRow class="text-center">
							<TableBodyCell colspan="4">No Data ...</TableBodyCell>
						</TableBodyRow>
					{:else}
						{#each AkunRekening as item (item.id)}
							<TableBodyRow class="text-center">
								<TableBodyCell tdClass="p-0 m-0"
									><a href="./rekening/kelompok/{item.id}">{item.kode}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a href="./rekening/kelompok/{item.id}">{item.uraianAkun}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0"
									><a href="./rekening/kelompok/{item.id}">{item.deskripsiAkun}</a></TableBodyCell
								>
								<TableBodyCell tdClass="p-0 m-0">
									<Button
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Akun Rekening</h3>
		...
		<Label class="space-y-2">
			<span>Uraian Akun Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="uraianAkun"
				placeholder="Masukkan Uraian Akun Rekening"
			/>
		...
		<Label class="space-y-2">
			<span>Deskripsi Akun Rekening</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
				type="text"
				name="deskripsiAkun"
				placeholder="Masukkan Deskripsi Akun Rekening"
			/>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Akun Rekening</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
