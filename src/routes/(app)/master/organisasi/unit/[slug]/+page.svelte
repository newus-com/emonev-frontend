<script lang="ts">
	...
	import { List, Create, Update, Delete } from '$lib/api/unit';
	import {
		Button,
		Modal,
		Label,
	...
	import { One } from '$lib/api/organisasi';
	...
	let myBreadItems = [
		...
		{ label: 'Unit', href: '/master/organisasi/unit/' + $page.params.slug }
	];
	...
	let stepperData: IStepper[] = [
		...
		{
			label: 'Unit',
			href: '/master/organisasi/unit/' + $page.params.slug
		}
	];
	...
	let dataTable = {
		search: {
			column: ['unit.kode', 'unit.nomenklatur'],
			value: ''
		},
		order: [
			{
	...
	async function getUnit() {
		const dataUnit = await List(dataTable);
		if (dataUnit.status == 200) {
			Unit = dataUnit.data.data.map((item: any) => ({
				id: item.id,
				kode: item.kode,
				nomenklatur: item.nomenklatur,
				organisasiId: item.organisasiId
			}));

			if (Unit.length != 0) {
				totalPages = dataUnit.data.totalData / dataTable.length;
				if (totalPages <= 1) {
					totalPages = 1;
				} else {
			...
			//notif
			if (typeof dataUnit.message !== 'undefined') {
				toasts.update((toastsList) => [
					...toastsList,
					{
						id: Date.now(),
						typeToast: 'error',
						messageToast: dataUnit.message,
						timeout: 3000
					}
				]);
			}
	...
	async function getOneOrganisasi(id: string) {
		const dataOrganisasi = await One(id);
		if (dataOrganisasi.status == 200) {
			Organisasi = {
				id: dataOrganisasi.data.id,
				bidangId: dataOrganisasi.data.bidangId,
				kode: dataOrganisasi.data.kode,
				nomenklatur: dataOrganisasi.data.nomenklatur
			};
		} else {
			//notif
	...
	onMount(async () => {
		isLoading = true;
		await getUnit();
		await getOneOrganisasi($page.params.slug);
		isLoading = false;
	});
	...
	async function onAdd(e: any) {
		...
		let responses: any = {};
		if (tambahModal) {
			data['organisasiId'] = $page.params.slug;
	...
	async function onEdit(e: any) {
		...
		if (editModal) {
			...
			data['organisasiId'] = editValue.organisasiId;
...
<section>
	...
	<div class="font-bold text-xl text-[#1E40AF] mb-2">Data Unit</div>
	<div class="wrap bg-white p-6 rounded-md shadow-md">
		...
		<div class="">
			<Blockquote border bg class="p-4 my-4">
				<P size="xs" height="relaxed"><b>Data unit dari organisasi:</b></P>

				<P size="xs" height="relaxed">
					"{Organisasi.kode} | {Organisasi.nomenklatur}"
				</P>
			</Blockquote>
...
<Modal bind:open={tambahModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onAdd} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Tambah Unit</h3>
...
<Modal bind:open={editModal} size="xs" autoclose={false} class="w-full">
	<form on:submit|preventDefault={onEdit} class="flex flex-col space-y-6" action="#">
		<h3 class="mb-2 text-xl font-medium text-gray-900 dark:text-white">Ubah Unit</h3>
		<Label class="space-y-2">
			<span>Kode</span>
			<Input
				class="focus:ring focus:ring-blue-300 focus:border-blue-500 mb-3 bg-white"
